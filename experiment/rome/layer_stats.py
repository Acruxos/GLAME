...
def main():
    ...
    aa("--sample_size", default=100000,
       type=lambda x: None if x == "all" else int(x))
    aa("--batch_tokens", default=None,
       type=lambda x: None if x == "any" else int(x))
    aa("--precision", default="float32",
...
def layer_stats(
    ...
    def get_ds():
        raw_ds = load_dataset(
            ds_name,
            dict(wikitext="wikitext-103-raw-v1",
                 wikipedia="20200501.en")[ds_name],
        )
    ...
    if not filename.exists() and download:
        ...
        try:
            print(
                f"Attempting to download {file_extension} from {remote_url}.")
